...
// MARK: - IssueDetailView (Temporary until Member 3 completes theirs)
// ⚠️ Delete and replace this entire file once Member 3 finishes IssueDetailView.swift
...
struct IssueDetailView: View {
    ...
    private var photoHeader: some View {
        ZStack(alignment: .topLeading) {
            ...
            HStack(spacing: 6) {
                Image(systemName: issue.category.symbolName).font(.system(size: 12, weight: .semibold))
                Text(issue.category.displayName).font(ZOOMINFont.captionBold)
            }
            .foregroundStyle(.white)
            .padding(.horizontal, 10).padding(.vertical, 5)
            .background(issue.category.markerColor).clipShape(Capsule())
            .padding(16)
            ...
            HStack {
                Spacer()
                Button {
                    // Share feature (future expansion)
                } label: {
    ...
    private var infoCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(issue.title).font(ZOOMINFont.title2).foregroundStyle(Color.textPrimary)
            HStack(spacing: 4) {
                Image(systemName: "mappin.and.ellipse").font(.system(size: 12)).foregroundStyle(Color.textSecondary)
                Text("Main St · 2.3 km away · Reported \(issue.reportDate.formatted(date: .abbreviated, time: .omitted))")
                    .font(ZOOMINFont.caption).foregroundStyle(Color.textSecondary)
            }
            HStack(spacing: 8) { ZOOMINStatusBadge(status: issue.status); ZOOMINRiskBadge(level: issue.safetyRisk) }
            if !issue.description.isEmpty {
                Divider()
                Text(issue.description).font(ZOOMINFont.body).foregroundStyle(Color.textSecondary).lineSpacing(4)
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading).zoominCard()
    }

    // Priority score card
    private var priorityCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Priority Score").font(ZOOMINFont.title3).foregroundStyle(Color.textPrimary)
            ...
            Text("Support Score is capped at 3 to prevent manipulation.")
                .font(ZOOMINFont.micro).foregroundStyle(Color.textTertiary)
        }
        .zoominCard()
    }
    ...
    private func priorityFactorCell(label: String, value: Int, max: Int, color: Color) -> some View {
        VStack(spacing: 4) {
            Text("\(value)").font(ZOOMINFont.title2).foregroundStyle(color)
            Text("/ \(max)").font(ZOOMINFont.micro).foregroundStyle(Color.textTertiary)
            Text(label).font(ZOOMINFont.micro).foregroundStyle(Color.textSecondary)
        }
        .frame(maxWidth: .infinity)
    }
    ...
    private var supportSection: some View {
        HStack(spacing: 16) {
            // Support button
            Button { issueStore.supportIssue(issueID: issue.id) } label: {
                HStack(spacing: 8) {
                    Image(systemName: "hand.thumbsup.fill").foregroundStyle(Color.zoominBlue)
                    Text("Support \(issue.supportCount)").font(ZOOMINFont.bodyBold).foregroundStyle(Color.zoominBlue)
                }
                .frame(maxWidth: .infinity).padding(.vertical, 14)
                .background(Color.zoominBlueLight)
                .clipShape(RoundedRectangle(cornerRadius: ZOOMINLayout.cornerRadiusLarge))
                .overlay(RoundedRectangle(cornerRadius: ZOOMINLayout.cornerRadiusLarge).stroke(Color.zoominBlue.opacity(0.3), lineWidth: 1))
            }
            // Points badge
    ...
    // Tab section (Details / Updates / Comments)
            ...
            HStack(spacing: 0) {
                ForEach(Array(detailTabs.enumerated()), id: \.offset) { idx, tab in
                    Button { withAnimation { selectedTab = idx } } label: {
                        VStack(spacing: 6) {
                            Text(tab).font(ZOOMINFont.captionBold)
                                .foregroundStyle(selectedTab == idx ? Color.zoominBlue : Color.textTertiary)
                            Rectangle().fill(selectedTab == idx ? Color.zoominBlue : Color.clear).frame(height: 2)
                        }
                    }
                    .frame(maxWidth: .infinity)
                }
    ...
    private var detailsTabContent: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            detailRow(icon: "calendar",           label: "Reported",   value: issue.reportDate.formatted(date: .long, time: .omitted))
            if let completion = issue.completionDate {
                detailRow(icon: "checkmark.seal.fill", label: "Completed", value: completion.formatted(date: .long, time: .omitted))
            }
            if let summary = issue.completionSummary {
                detailRow(icon: "doc.text.fill",  label: "Summary",    value: summary)
            }
        }
    }
    ...
    private func detailRow(icon: String, label: String, value: String) -> some View {
        HStack(spacing: 10) {
            Image(systemName: icon).font(.system(size: 13)).foregroundStyle(Color.zoominBlue).frame(width: 20)
            Text(label).font(ZOOMINFont.captionBold).foregroundStyle(Color.textSecondary).frame(width: 70, alignment: .leading)
            Text(value).font(ZOOMINFont.caption).foregroundStyle(Color.textPrimary)
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ForEach(IssueStatus.allCases, id: \.self) { st in
                HStack(spacing: 12) {
                    // Timeline dot
                    ZStack {
                        Circle().stroke(st.badgeColor.opacity(0.3), lineWidth: 2).frame(width: 28, height: 28)
                        if issue.status == st || statusOrder(st) < statusOrder(issue.status) {
                            Circle().fill(st.badgeColor).frame(width: 20, height: 20)
                            Image(systemName: st.symbolName).font(.system(size: 9, weight: .bold)).foregroundStyle(.white)
                        }
                    }
                    VStack(alignment: .leading, spacing: 2) {
                        Text(st.displayName).font(ZOOMINFont.captionBold)
                            .foregroundStyle(statusOrder(st) <= statusOrder(issue.status) ? st.badgeColor : Color.textTertiary)
                        if issue.status == st {
                            Text("Current Status").font(ZOOMINFont.micro).foregroundStyle(Color.textSecondary)
                        }
                    }
                }
            }
    ...
    private func statusOrder(_ s: IssueStatus) -> Int {
        switch s {
        case .received: return 0; case .reviewing: return 1
        case .inProgress: return 2; case .completed: return 3
        }
    }

    private var commentsPlaceholder: some View {
        ZOOMINEmptyStateView(mood: .thinking, title: "No Comments", message: "No comments yet.")
    }
}
